...
MouseMotionListener {
	...
	private JComboBox selectedCatBox;
	...
	public GraphPanel(JComboBox selectedCatBox) {
		this.selectedCatBox = selectedCatBox;
		setScale(0, 1, 0, 1);
		setMargins(15, 10, 20, 10);
	...
	public void setCutPoints(double[] _cutPoints){
		...
		if (dist != null) {
			...
			if (type == DISCRETE) {
				...
				for (int i=0 ; i < cutPoints.length ; i++){
					b = cutPoints[i];
					if (fill || selectedCatBox.getSelectedIndex() == i){
						if (selectedCatBox.getSelectedIndex() == i)
				...
				b = right;
				if (fill || selectedCatBox.getSelectedIndex() == cutPoints.length){
					if (selectedCatBox.getSelectedIndex() == cutPoints.length)
						g.setColor(selectionColor);
					for (x = a; x < b; x = x + delta) {
						drawLine(g, x, dist.getDensity(x), x, 0);
			...
			if (dist.getType()== CONTINUOUS) {
				...
				//g.drawString(str, xGraph(((int)(100*left))/100)-w/2, yGraph(0)+h);
				for (double cut : cutPoints){
					str = Double.toString((1.0*((int)(100*cut)))/100);
					g.drawString(str, xGraph(cut)-w/2, yGraph(0) + h);					
				}
	...
	public void mousePressed(MouseEvent evt) {
		...
		int cat=0;
		for (int i=0 ; i < cutPoints.length ; i++){
			if (xScale(x) < cutPoints[i]){
				cat = i;
				break;
			}
		}
		if (cutPoints.length > 0 && xScale(x) > cutPoints[cutPoints.length-1]) cat = cutPoints.length;
		selectedCatBox.setSelectedIndex(cat);
		repaint();
	}
	...
	public double getLeftCDF() {
		// //System.out.println("left="+left+" CFD(left)="+dist.getCDF(left)+
		// " xPosition="+xPosition);
		if (type==DISCRETE) return dist.getCDF(left-0.5);
		else return dist.getCDF(left);
	}

	public double getRightCDF() {
		if (type==DISCRETE) return 1 - dist.getCDF(right-0.5);
		else return 1 - dist.getCDF(right);
	}

	public double getBetweenCDF() {
		return 1 - getLeftCDF() - getRightCDF();
	}
